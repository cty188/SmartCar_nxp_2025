...
class BalanceCarController:
    ...
    def __init__(self):
        ...
        self.angle_pid = PIDController(
            ...
            Kp=0.080,     # 角度环比例增益（值越大，恢复平衡越强）
            Ki=0.0012,     # 角度环积分增益
            Kd=0.003,     # 角度环微分增益（减小过冲）
            max_out=10.0, # 最大输出速度(m/s)
            max_iout=5.0 # 最大积分输出
        )
        self.base_target_pitch=5.1  # 基础平衡俯仰角
    ...
    def update_balance_control(self):
        ...
        if dtype == "raw":
            ...
            printf(debug_uart, f": {filtered_pitch:.3f}", print_console=False)
        elif dtype == "speed":
            # 速度
            #if self.debug_enabled and self.loop_count % self.debug_interval == 0:
